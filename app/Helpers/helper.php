...
if (! function_exists('getEnabledPaymentMethods')) {
    ...
    {
        $methods = ['stripe', 'paypal', 'razorpay', 'mercadopago', 'paystack', 'flutterwave', 'bank', 'paytabs', 'skrill', 'coingate', 'payfast', 'tap', 'xendit', 'paytr', 'mollie', 'toyyibpay', 'cashfree', 'iyzipay', 'benefit', 'ozow', 'easebuzz', 'khalti', 'authorizenet', 'fedapay', 'payhere', 'cinetpay', 'midtrans'];
        $enabled = [];
...
if (! function_exists('getSidebarLogo')) {
    function getSidebarLogo()
    {
        ...
        // add app url if logo path is relative
        if (!Str::startsWith($logoPath, ['http://', 'https://'])) {
            $logoPath = config('app.url') . $logoPath;
        }
        return $logoPath;
    }
}
...
if (! function_exists('getFile')) {
    ...
    {
        // Return path as is if it's already a full URL
...
if (! function_exists('copySettingsFromSuperAdmin')) {
    ...
    function copySettingsFromSuperAdmin($companyUserId, $workspaceId)
    {
        ...
        // Set company user's lang field from superadmin's defaultLanguage setting
        $defaultLanguage = $superAdminSettings->firstWhere('key', 'defaultLanguage');
        if ($defaultLanguage) {
    ...
    if (! function_exists('isNotificationTemplateEnabled')) {
        ...
        function isNotificationTemplateEnabled($templateName, $userId = null, $type = null)
        {
            ...
            $query = UserNotificationTemplate::where('workspace_id', $workspaceId)
                ->where('template_id', $template->id);
...
if (! function_exists('check_file')) {
    function check_file($path)
    {
        try {
            ...
            // Return true if it's already a full URL
            if (str_starts_with($path, 'http') || str_starts_with($path, 'https')) {
                return true;
            }
