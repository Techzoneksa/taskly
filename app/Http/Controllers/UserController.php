...
class UserController extends BaseController
{
    ...
    public function store(UserRequest $request)
    {
        ...
        // Get company's current workspace defaultLanguage setting
        $companyUser = in_array($authUser->type, ['superadmin', 'company']) ? $authUser : User::find($authUser->created_by);
        $defaultLanguage = \App\Models\Setting::where('user_id', $companyUser?->id)
            ->where('workspace_id', $companyUser?->current_workspace_id)
            ->where('key', 'defaultLanguage')
            ->value('value');
        $userLang = $defaultLanguage ?: ($authUser->lang ?: 'en');
