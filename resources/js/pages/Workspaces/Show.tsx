...
export default function Show({ workspace, availableRoles, errors, isSaasMode = true }: Props) {
    ...
    const getRoleColor = (role: string) => {
        switch (role) {
            case 'owner': return 'bg-amber-50 text-amber-700 ring-1 ring-inset ring-amber-600/20';
            case 'admin': return 'bg-red-50 text-red-700 ring-1 ring-inset ring-red-600/20';
            case 'manager': return 'bg-violet-50 text-violet-700 ring-1 ring-inset ring-violet-600/20';
            case 'member': return 'bg-blue-50 text-blue-700 ring-1 ring-inset ring-blue-600/20';
            case 'client': return 'bg-green-50 text-green-700 ring-1 ring-inset ring-green-600/20';
                    ...
                    <CardContent>
                        <form onSubmit={handleSettingsUpdate} className="space-y-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <Label required htmlFor="name">{t('Workspace Name')}</Label>
                                    <Input
                                        id="name"
                                        type="text"
                    ...
                    <CardContent>
                        ...
                        <form onSubmit={handleInvite} className="space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <Label required htmlFor="email">{t('Email Address')}</Label>
                                    <Input
                                        id="email"
                                        type="email"
                                        value={data.email}
                                ...
                                <div>
                                    <Label required htmlFor="role">{t('Role')}</Label>
                                    <Select
                                        value={data.role}
                    ...
                    <CardContent>
                        <div className="space-y-3">
                            {workspace.members.map((member) => (
                                <div key={member.id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-gray-50 transition-colors dark:hover:bg-gray-700/50">
                                    <div className="flex items-center gap-3">
                                        <Avatar className="w-10 h-10">
                                            {member.user.avatar && (
                                        ...
                                        <div>
                                            ...
                                            <div className="flex items-center gap-1 mt-1 text-xs text-gray-400">
                                                ...
                                                {t('Joined')} {window.appSettings.formatDateTime(new Date(member.joined_at),false)}
                                            </div>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <TooltipProvider>
                                            <Tooltip>
                                                <TooltipTrigger asChild>
                                                    <span className={`inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ${getRoleColor(member.role)}`}>
                                        ...
                                        <span className={`inline-flex items-center capitalize rounded-md px-2 py-1 text-xs font-medium ${member.status === 'active' ? 'bg-green-50 text-green-700 ring-1 ring-inset ring-green-600/20' : 'bg-gray-50 text-gray-700 ring-1 ring-inset ring-gray-600/20'}`}>
                                            {member.status}
                                        </span>
                                ...
                                <div key={invitation.id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-gray-50 transition-colors">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <div className="flex items-center gap-1 mt-1 text-xs text-gray-400">
                                                <Clock className="w-3 h-3" />
                                                {t('Expires')}: {window.appSettings.formatDateTime(new Date(invitation.expires_at),false)}
                                            </div>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <span className={`inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ${getRoleColor(invitation.role)}`}>
